...
func (dn *DataNode) sendHeartbeats() {
	// Initial delay to stagger heartbeats
...
func (dn *DataNode) storeBlock(payload common.StoreBlockPayload) {
	...
	// Step 5: Forward to next in replication pipeline
...
func (dn *DataNode) forwardBlock(addr string, block common.Block, fileName string, targets []string, targetAddrs []string) {
	...
	if err != nil {
		...
		dn.log.Error("   ⚠️ Replication factor may be reduced for this block!")
		return
	}
	defer conn.Close()
	...
	if !exists {
		...
		if err == nil {
			...
			if !common.VerifyChecksum(data, string(expectedChecksum)) {
				...
				dn.log.Error("   Expected checksum: %s", string(expectedChecksum))
				dn.log.Error("   Actual checksum:   %s", common.ComputeChecksum(data))
...
func (dn *DataNode) handleConnection(conn net.Conn) {
	...
	case common.MsgReadFile:
		...
		dn.log.Info("📖 Block read request: %s", readReq.FileName)
...
func main() {
	...
	if rack == "" {
		rack = common.RackB // DataNode1 is in Rack B by default
	}
	...
	dn.log.BannerWithRack("DATANODE (Rack B)", rack)
